...
def predict_values(data, goals, net):
    ...
    state_values = net(torch.cat([states, goals], dim=1)).max(1)
    return state_values
...
def plot_values(net, goal):
    ...
    z = predict_values(xy, goals, net)[0].data.numpy().reshape(X.shape)
    plt.pcolor(X, Y, z)
    plt.show()
...
policy_net, target_net = fitted_q(data, policy_net, target_net, 5000)
